...
// Server component — pure markup. The fade-in stagger and hover lift
// are pure CSS in globals.css (.hero-tile / @keyframes hero-tile-in),
// so there's no client-side JS for this whole grid.
      ...
      <div className="absolute inset-4 bg-surface-50 rounded-2xl border border-surface-200 overflow-hidden p-5">
        <div className="grid grid-cols-4 gap-3 h-full content-center">
          {TOOLS.map((tool, i) => (
            <div
              key={tool.name}
              className="hero-tile flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-surface-100 border border-surface-200 hover:border-brand-500/30 cursor-default aspect-square"
